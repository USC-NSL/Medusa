...
package medusa.medusalet.genmetadata;
...
import medusa.mobile.client.MedusaOpencv;
...
public class MedusaletMain extends MedusaletBase 
{
    ...
    public boolean run() {
    	...
    	Time now = new Time();
    	now.setToNow();
    	long earliest = now.toMillis(false) - 1000*beforeSeconds;
    	...
    	String statement = "select uid,type,path,mtime,lat,lng,fsize from mediameta where type='image' and uid>=" + start_uid;
